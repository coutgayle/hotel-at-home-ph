...
function ViewBookingContent() {
  ...
  // Enforce access only from Book Now page
  useEffect(() => {
    if (searchParams.get('from') !== 'book-now') {
      router.replace('/book-now');
    }
  }, [searchParams, router]);

  // If the query parameter doesn't match, return null to prevent rendering the UI
  if (searchParams.get('from') !== 'book-now') {
    return null;
  }
  ...
  return (
    <main className="min-h-[calc(100vh-80px)] bg-[#f3f6fb] text-brand-blue pb-20 pt-12 sm:pt-24 px-6 relative">
      ...
      <div className="mx-auto max-w-xl mt-2 sm:mt-0">
        <div className="mb-8 sm:hidden">
          <button onClick={() => router.push('/book-now')} className="flex items-center gap-2 text-sm font-semibold text-brand-blue/70 hover:text-brand-blue transition">
        ...
        <div className="text-center mb-10">
          ...
          <p className="mt-3 text-brand-blue/70">Enter your order code and email below to view your details.</p>
        </div>

        <div className="bg-white rounded-[32px] p-8 shadow-sm border border-brand-blue/5">
          <form onSubmit={handleLookup} className="space-y-4">
            <div>
              <input type="text" placeholder="e.g. HH-ABC123" value={code} onChange={(e) => setCode(e.target.value)} className="w-full rounded-xl border border-brand-blue/10 bg-[#f3f6fb] px-6 py-4 outline-none focus:border-brand-blue transition text-center text-lg uppercase tracking-widest font-semibold placeholder:font-normal placeholder:tracking-normal" required />
            </div>
            <div>
              <input type="email" placeholder="Email address used for booking" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded-xl border border-brand-blue/10 bg-[#f3f6fb] px-6 py-4 outline-none focus:border-brand-blue transition text-center text-base placeholder:text-brand-blue/50" required />
            </div>
            {error && <p className="text-red-500 text-sm text-center">{error}</p>}
            <button type="submit" disabled={isLoading || !code.trim() || !email.trim()} className="w-full rounded-full bg-brand-blue px-6 py-4 font-semibold text-white transition hover:bg-[#001a72] disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        {booking && (
          <div className="mt-8 bg-white rounded-[32px] p-8 shadow-sm border border-brand-blue/5 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid sm:grid-cols-2 gap-6 text-sm">
              ...
              {booking.room_id === 3 ? (
                <div className="sm:col-span-2"><p className="text-xs uppercase tracking-wider text-brand-blue/50 mb-1">Event Purpose</p><p className="font-medium whitespace-pre-wrap">{booking.purpose || 'N/A'}</p></div>
              ) : (
...
export default function ViewBookingPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-brand-blue font-semibold">Loading...</div>}>
      <ViewBookingContent />
    </Suspense>
  );
}